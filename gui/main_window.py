...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.btn_minimized.setCheckable(True)
        self.btn_fullscreen.setEnabled(False)
        # 设置按钮颜色
        self.btn_close.setStyleSheet(qss_cfg.BTN_COLOR_RED)
        self.btn_minimized.setStyleSheet(qss_cfg.BTN_COLOR_GREEN)
        self.btn_fullscreen.setStyleSheet(qss_cfg.BTN_COLOR_GRAY)
        ...
        self.left_label_about = QPushButton(qtawesome.icon('fa.user-circle-o', color='white'), " About")

        for label in (
                self.left_label_android,
                self.left_label_ios,
                self.left_label_fastbot,
                self.left_label_about
        ):
        ...
        self.left_layout.addStretch(1)
        self.left_layout.addWidget(self.left_label_about)
        ...
        self.left_label_about.clicked.connect(lambda: self.switch_about())
    ...
    def switch_about(self):
        self.right_stacked.setCurrentIndex(3)
...
def main():
    ...
    gui = MainWindow()
    gui.setWindowTitle(config.APP_NAME)
    gui.show()
    sys.exit(app.exec_())
